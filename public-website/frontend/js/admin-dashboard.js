document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (!session) {
        // Double check cookie with stats call
        try {
    ...
    async function loadJobs() {
        try {
            ...
            // Populate filter in applicants tab while we're at it
...
async function updateAppStatus(id, status) {
    try {
        ...
        window.loadApplicants(); // Refresh list
    } catch {
...
function openJobForm(jobId = null) {
    ...
    if (jobId) {
        // Fetch specific job data for editing if needed (optional optimization)
    }
